...
def cmd_add(args):
    ...
    if args.username in users:
        ...
        sys.exit(1)
    if args.role not in ("admin", "worker"):
        print(f"❌ Rola musi być 'admin' lub 'worker' (podałeś: {args.role})")
        sys.exit(1)
...
def cmd_role(args):
    ...
    if args.username not in users:
        ...
        sys.exit(1)
    if args.role not in ("admin", "worker"):
        print(f"❌ Rola musi być 'admin' lub 'worker'")
        sys.exit(1)
...
def main():
    ...
    p_add.add_argument("--role", default="worker", choices=["admin", "worker"])
    ...
    p_role.add_argument("role", choices=["admin", "worker"])
